 ...
 *   Optional: data-scroll-frame-overlap=".panel" Ã¢â‚¬â€ after blur completes, panel fades in + slides up (was hidden until then)
 ...
 *   Optional: data-scroll-frame-pin-spacing Ã¢â‚¬â€ default off: pinSpacing false (no extra pin-spacer padding; content below keeps natural layout; triggers after init recalc via global refresh). Use "true"/"1"/"yes" for GSAP default pin spacing, or "margin" for pinSpacing "margin".
 ...
 * - data-scroll-tags-marquee     : tag row; horizontal shift scrubbed to scroll. Optional: data-scroll-tags-track, data-scroll-tags-scroll-start / data-scroll-tags-scroll-end (defaults: top bottom Ã¢â€ â€™ bottom top). data-scroll-tags-fullwidth="1" for 100vw breakout (off by default Ã¢â‚¬â€ avoids parent overflow clip). data-scroll-tags-scrub (default 1.35)
 * - data-scroll-horizontal       : horizontal scroll section Ã¢â‚¬â€ pins the wrapper and translates its track child left as user scrolls vertically.
 ...
 * - data-magnetic-hover           : cursor pull via translate3d + lerp (vanilla rAF, no ScrollTrigger). Optional: data-magnetic-strength (0Ã¢â‚¬â€œ1, default 0.35) | data-magnetic-lerp (default 0.12). Do not combine with GSAP transform on the same node.
 *   Demo root [data-magnetic-hover-root] + input[data-magnetic-strength-slider] (0Ã¢â‚¬â€œ100) adjusts strength for child [data-magnetic-hover] nodes.
 * - data-scroll-parallax          : vertical parallax (vanilla scroll). Optional: data-scroll-parallax-scroll (selector, default window)
 * - data-scroll-octagon-reveal    : pin container; .reveal-wrapper grows small octagon Ã¢â€ â€™ fullscreen (scrub). Also matches .animation-container.
...
(function () {
	...
	function initSnapScrolling() {
		...
		// â”€â”€â”€ Section positions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
		...
		// â”€â”€â”€ Snap animation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
		var snapTween     = null;
		var snapCoolMs    = 0;   // block new snaps during cooldown
		var wheelBlockMs  = 0;   // block ESS wheel events

		// Anti-ping-pong: track last landing position + time
		// Prevents re-snapping to same target within RESNAPGUARD ms
		var lastLandedY   = -9999;
		var lastLandedMs  = 0;
		var RESNAPGUARD   = 5000; // 5 seconds
		...
		// Block ESS wheel events during snap animation only
		...
		// â”€â”€â”€ rAF detection loop â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
		var rafY       = getY();
		var startupMs  = Date.now() + 2500; // wait for ESS to settle on load

		// Backward snap: need 120px past section boundary + slow velocity
		var BACK_THRESH  = 120; // px above section start
		var BACK_MAX_V   = 28;  // max px/frame for backward snap

		(function rafCheck() {
			requestAnimationFrame(rafCheck);

			var now = Date.now();
			if (now < startupMs)  { rafY = getY(); return; } // still warming up
			if (snapTween)        { rafY = getY(); return; } // snap animation running
			if (now < snapCoolMs) { rafY = getY(); return; } // cooldown
			...
			if (absV < 1.5) return; // not moving enough

			var winH = window.innerHeight;
			var idx  = getSectionIdx(y);

			if (dy > 0) {
				// â”€â”€ FORWARD: snap when viewport bottom passes section end â”€â”€
				if (y + winH >= snapEnds[idx] - 5 && idx < sections.length - 1) {
					var nextTarget = snapStarts[idx + 1];
					// Anti-ping-pong: skip if we just landed here
					if (Math.abs(nextTarget - lastLandedY) < 10
						&& now - lastLandedMs < RESNAPGUARD) return;
					doSnap(y, nextTarget);
				}
			} else {
				// â”€â”€ BACKWARD: only snap when slow + past threshold â”€â”€â”€â”€â”€â”€â”€â”€
	...
	function initAll() {
		...
		requestAnimationFrame(function () {
			requestAnimationFrame(function () {
				...
				ScrollTrigger.refresh();
				// initSnapScrolling chÃ¡ÂºÂ¡y trong window.load (sau khi mÃ¡Â»Âi thÃ¡Â»Â© Ã¡Â»â€¢n Ã„â€˜Ã¡Â»â€¹nh)
			});
		});
	}
	...
	window.addEventListener('load', async function () {
		...
		// KhÃ¡Â»Å¸i Ã„â€˜Ã¡Â»â„¢ng snap SAU KHI tÃ¡ÂºÂ¥t cÃ¡ÂºÂ£ Ã„â€˜ÃƒÂ£ Ã¡Â»â€¢n Ã„â€˜Ã¡Â»â€¹nh:
		// fonts Ã„â€˜ÃƒÂ£ tÃ¡ÂºÂ£i, images Ã„â€˜ÃƒÂ£ cÃƒÂ³ kÃƒÂ­ch thÃ†Â°Ã¡Â»â€ºc, ScrollTrigger Ã„â€˜ÃƒÂ£ refresh xong.
		// LÃƒÂºc nÃƒÂ y getDocTop() mÃ¡Â»â€ºi cho vÃ¡Â»â€¹ trÃƒÂ­ chÃƒÂ­nh xÃƒÂ¡c.
		requestAnimationFrame(function () {
			initSnapScrolling();
		});
	});
