...
(function () {
	...
	function initSnapScrolling() {
		...
		console.log('[Snap v15] init y=', scrollFn());
		...
		console.log('[Snap v15] starts:', JSON.stringify(snapStarts));
		console.log('[Snap v15] ends:  ', JSON.stringify(snapEnds));
		...
		function doSnap(fromY, targetY) {
			...
			console.log('[Snap v15]', Math.round(fromY), '->', Math.round(targetY));
			var proxy = { y: fromY };
			snapTween = gsap.to(proxy, {
				y: targetY, duration: 0.45, ease: 'power2.out',
				onUpdate: function () { scrollFn(proxy.y); },
				onComplete: function () {
					snapTween    = null;
					lastLandedY  = targetY;
					lastLandedMs = Date.now();
					console.log('[Snap v15] landed', Math.round(scrollFn()));
				},
			});
		}

		// Velocity-window rAF loop
		// Only fire snap when ESS momentum is in range: MIN_V <= absV <= MAX_V
		//   - absV < MIN_V : not moving, nothing to snap
		//   - absV > MAX_V : ESS at full speed, don't fight it (causes bounce)
		//   - MIN_V <= absV <= MAX_V : ESS decelerating, safe to snap without conflict
		var rafY      = getY();
		var startupMs = Date.now() + 2500;
		var MIN_V     = 1;    // px/frame minimum
		var MAX_V     = 30;   // px/frame maximum (don't snap at high speed)
		var BACK_THRESH = 100;
		...
		(function rafCheck() {
			...
			// Only act within velocity window
			if (absV < MIN_V || absV > MAX_V) return;

			var winH = window.innerHeight;
			var idx  = getSectionIdx(y);

			if (dy > 0) {
				// Forward: snap only if crossing section boundary at safe speed
				if (y + winH >= snapEnds[idx] - 5 && idx < sections.length - 1) {
					var nt = snapStarts[idx + 1];
					if (Math.abs(nt - lastLandedY) < 10 && now - lastLandedMs < RESNAPGUARD) return;
					doSnap(y, nt);
				}
			} else {
				// Backward: snap when past boundary threshold at safe speed
				if (y < snapStarts[idx] - BACK_THRESH && idx > 0) {
					var pt = snapStarts[idx - 1];
					if (Math.abs(pt - lastLandedY) < 10 && now - lastLandedMs < RESNAPGUARD) return;
					doSnap(y, pt);
				}
			}
		})();
	}
	// Snap scrolling - End (v15)
