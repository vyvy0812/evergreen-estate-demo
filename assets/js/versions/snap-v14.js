...
(function () {
	...
	/** ScrollTrigger update ??? smooth scroll module d??ng gsap.ticker khi b???t. */
	...
	function initSnapScrolling() {
		...
		console.log('[Snap v14] init y=', scrollFn());
		...
		console.log('[Snap v14] starts:', JSON.stringify(snapStarts));
		console.log('[Snap v14] ends:  ', JSON.stringify(snapEnds));
		...
		var lastLandedMs= 0;
		var RESNAPGUARD = 8000;

		function doSnap(fromY, targetY) {
			if (snapTween) { snapTween.kill(); snapTween = null; }
			snapCoolMs = Date.now() + 400;
			console.log('[Snap v14]', Math.round(fromY), '->', Math.round(targetY));
			var proxy = { y: fromY };
			snapTween = gsap.to(proxy, {
				...
				onComplete: function () {
					...
					console.log('[Snap v14] landed', Math.round(scrollFn()));
				},
			});
		}

		// rAF loop
		// Strategy: DEFERRED SNAP
		//   Step 1 - Detect: when viewport crosses a section boundary, record pendingTarget
		//   Step 2 - Wait:   do nothing while ESS is at high momentum (absV > EXEC_V)
		//   Step 3 - Execute: once ESS decelerates (absV <= EXEC_V), fire the snap cleanly
		// Result: snap never fights ESS at full speed -> no jerk/stutter
		var rafY          = getY();
		var startupMs     = Date.now() + 2500;
		var pendingFwd    = null; // forward snap target waiting to execute
		var pendingBwd    = null; // backward snap target waiting to execute
		var EXEC_V        = 8;   // px/frame: execute snap only when ESS this slow
		var BACK_THRESH   = 120;
		var BACK_MAX_V    = 28;
		...
		(function rafCheck() {
			...
			var now = Date.now();
			if (now < startupMs) { rafY = getY(); pendingFwd = null; pendingBwd = null; return; }
			...
			if (dy > 0.3) {
				// Moving forward: check if we crossed a section end
				if (idx < sections.length - 1 && y + winH >= snapEnds[idx] - 5) {
					var nt = snapStarts[idx + 1];
					var guardOk = !(Math.abs(nt - lastLandedY) < 10 && now - lastLandedMs < RESNAPGUARD);
					if (guardOk) pendingFwd = nt;
				} else {
					// Moved away from boundary going forward - clear stale pending
					if (pendingFwd !== null && y + winH < snapEnds[idx] - 80) pendingFwd = null;
				}
				pendingBwd = null; // changed to fwd direction, clear bwd pending
			} else if (dy < -0.3) {
				// Moving backward: set pending bwd if conditions met
				if (absV < BACK_MAX_V && y < snapStarts[idx] - BACK_THRESH && idx > 0) {
					var pt = snapStarts[idx - 1];
					if (!(Math.abs(pt - lastLandedY) < 10 && now - lastLandedMs < RESNAPGUARD)) {
						pendingBwd = pt;
					}
				}
				pendingFwd = null; // changed to bwd direction, clear fwd pending
			}

			// ── STEP 2+3: Execute when ESS decelerates ────────────────────
			if (absV <= EXEC_V) {
				if (pendingFwd !== null) {
					var cur = getY();
					// Only snap forward if user hasn't overshot the target.
					// If ESS carried them past pendingFwd already, cancel - no bounce.
					if (cur < pendingFwd) {
						doSnap(cur, pendingFwd);
					}
					pendingFwd = null;
					return;
				}
				if (pendingBwd !== null) {
					var cur2 = getY();
					doSnap(cur2, pendingBwd);
					pendingBwd = null;
				}
			}
		})();
	}
	// Snap scrolling - End (v14)
	...
	window.addEventListener('load', async function () {
		...
		// Init snap AFTER fonts + images loaded + ScrollTrigger refreshed
		requestAnimationFrame(function () {
			initSnapScrolling();
		});
	});
