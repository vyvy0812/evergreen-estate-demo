 ...
 *   Optional: data-scroll-frame-overlap=".panel" â€” after blur completes, panel fades in + slides up (was hidden until then)
 ...
 *   Optional: data-scroll-frame-pin-spacing â€” default off: pinSpacing false (no extra pin-spacer padding; content below keeps natural layout; triggers after init recalc via global refresh). Use "true"/"1"/"yes" for GSAP default pin spacing, or "margin" for pinSpacing "margin".
 ...
 * - data-scroll-tags-marquee     : tag row; horizontal shift scrubbed to scroll. Optional: data-scroll-tags-track, data-scroll-tags-scroll-start / data-scroll-tags-scroll-end (defaults: top bottom â†’ bottom top). data-scroll-tags-fullwidth="1" for 100vw breakout (off by default â€” avoids parent overflow clip). data-scroll-tags-scrub (default 1.35)
 * - data-scroll-horizontal       : horizontal scroll section â€” pins the wrapper and translates its track child left as user scrolls vertically.
 ...
 * - data-magnetic-hover           : cursor pull via translate3d + lerp (vanilla rAF, no ScrollTrigger). Optional: data-magnetic-strength (0â€“1, default 0.35) | data-magnetic-lerp (default 0.12). Do not combine with GSAP transform on the same node.
 *   Demo root [data-magnetic-hover-root] + input[data-magnetic-strength-slider] (0â€“100) adjusts strength for child [data-magnetic-hover] nodes.
 * - data-scroll-parallax          : vertical parallax (vanilla scroll). Optional: data-scroll-parallax-scroll (selector, default window)
 * - data-scroll-octagon-reveal    : pin container; .reveal-wrapper grows small octagon â†’ fullscreen (scrub). Also matches .animation-container.
...
(function () {
	...
	/** ScrollTrigger update â€” smooth scroll module dÃ¹ng gsap.ticker khi báº­t. */
	...
	/** Pin scroll distance = innerHeight Ã— mult (viewport heights). data-scroll-frame-length, default 1.85. */
	...
	function initImageBlurOnScroll(imageEl) {
		...
		//   blur    : blur 0 â†’ blurPx (smooth, ~4 scroll-chunks worth of distance)
		//   hold    : sit at peak blur so the user feels the climax
		//   fade    : opacity 1 â†’ 0.7 (single continuous tween, no plateau)
		//
		...
		// when the user scrubs back and forth â€” avoids the "snap" you get with to()
		...
		// with every scroll delta â€” no boundary re-init glitches.
		tl.fromTo(
			imageEl,
			{ filter: 'blur(0px)' },
			{
		...
		// Single opacity tween â€” same pattern as blur (one continuous fromTo, linear
	 ...
	 * Optional: data-scroll-frame-overlap â€” panel hidden (autoAlpha 0) until blur ends, then fades in while sliding up. Class `independent-frame-overlap-panel` anchors bottom.
	 *
	 * @param {HTMLElement} frameEl
	...
	/**
	 * Image reveal by clip-path (top â†’ bottom) + slight lift.
	 * @param {HTMLElement} el
	 */
	...
	/**
	 * Split text masking by line â€” light stagger, power4.out, 0.8s per line.
	 ...
	 * Default scroll range: enter viewport (top bottom) â†’ leave (bottom top). Full-bleed 100vw only if data-scroll-tags-fullwidth="1".
	 *
	 * @param {HTMLElement} root
	 */
	 ...
	 *   data-scroll-horizontal-track  â€” CSS selector for track child (default: first child)
	 *   data-scroll-horizontal-ease   â€” GSAP ease for momentum coast (default "power3.out")
	 *   data-scroll-horizontal-speed  â€” momentum multiplier (default 1.8)
	 *
	...
	/** @param {string|null} raw @param {number} fallback 0â€“1 */
	...
	/** Magnetic hover â€” shared rAF pool (isolated from GSAP timelines). */
	...
	/**
	 * Octagon reveal â€” pin container, scrub width/height of .reveal-wrapper.
