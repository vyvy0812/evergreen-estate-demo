...
(function () {
	...
	function initSnapScrolling() {
		...
		console.log('[Snap v13] init y=', scrollFn());
		...
		console.log('[Snap v13] starts:', JSON.stringify(snapStarts));
		console.log('[Snap v13] ends:  ', JSON.stringify(snapEnds));
		...
		function doSnap(fromY, targetY) {
			...
			console.log('[Snap v13]', Math.round(fromY), '->', Math.round(targetY));
			var proxy = { y: fromY };
			snapTween = gsap.to(proxy, {
				...
				onComplete: function () {
					...
					console.log('[Snap v13] landed', Math.round(scrollFn()));
				},
			});
		}
