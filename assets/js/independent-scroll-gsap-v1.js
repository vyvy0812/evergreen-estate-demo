...
(function () {
	...
	function initSnapScrolling() {
		...
		console.log('[Snap v16] init y=', scrollFn());
		...
		console.log('[Snap v16] starts:', JSON.stringify(snapStarts));
		console.log('[Snap v16] ends:  ', JSON.stringify(snapEnds));
		...
		function doSnap(fromY, targetY) {
			...
			snapCoolMs = Date.now() + 500;
			console.log('[Snap v16]', Math.round(fromY), '->', Math.round(targetY));
			var proxy = { y: fromY };
			snapTween = gsap.to(proxy, {
				...
				onComplete: function () {
					...
					console.log('[Snap v16] landed', Math.round(scrollFn()));
				},
			});
		}

		// rAF loop - FORWARD SNAP ONLY
		// No backward snap: user can freely scroll up/down within sections.
		// Backward snapping caused mid-section drift when user tried to re-read content.
		//
		// Forward snap fires when:
		//   - Scrolling forward (dy > 0)
		//   - Velocity in safe window: MIN_V <= absV <= MAX_V
		//     (too fast = fights ESS causing bounce; too slow = already stopped)
		//   - Viewport bottom has reached section end
		var rafY      = getY();
		...
		(function rafCheck() {
			...
			// Only forward snap, only in safe velocity window
			if (dy <= 0) return;
