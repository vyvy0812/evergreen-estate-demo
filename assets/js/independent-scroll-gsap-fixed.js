...
(function () {
	...
	function initSnapScrolling() {
		...
		// Lấy vị trí tuyệt đối trong DOM (không phụ thuộc scroll hay CSS transform).
		// Đây là cách duy nhất đúng khi EvergreenSmoothScroll dùng transform —
		// getBoundingClientRect() + window.scrollY sẽ cho kết quả sai.
		...
		// Vị trí cuộn mượt hiện tại (của EvergreenSmoothScroll, không phải window.scrollY).
		// smoothY = docTop(sec) - rect.top(sec)  với bất kỳ section nào.
		function getSmoothY() {
			var sec = sections[0];
			var top = getDocTop(sec);
			var rect = sec.getBoundingClientRect();
			return top - rect.top;
		}

		// Ghi nhớ vị trí DOM tuyệt đối của từng section.
		// Khoảng cách giữa snapPts[i] và snapPts[i+1] = toàn bộ scroll distance
		// của section i, kể cả phần GSAP pin (vì pin-spacers đã được tạo trước đó).
		var snapPts = sections.map(getDocTop);
		...
		function getCurrentIdx() {
			var smoothY = getSmoothY();
			var viewMid = smoothY + window.innerHeight / 2;
			var idx = 0;
			for (var i = 0; i < snapPts.length; i++) {
				if (snapPts[i] <= viewMid) {
					idx = i;
				}
			}
			return idx;
		...
		window.addEventListener('wheel', function (e) {
			...
			if (isAnimating || now - lastSnapTime < cooldown) {
				e.preventDefault();
				return;
			}
			...
			if (e.deltaY > 0) {
				// Cuối section hiện tại = đầu section kế (bao gồm cả pin-spacer của GSAP)
				var sectionEnd = (idx < snapPts.length - 1)
					? snapPts[idx + 1]
					: document.documentElement.scrollHeight;

				if (smoothY + winH < sectionEnd - 15) {
					// Vẫn còn nội dung phía dưới → cuộn tự do
					wheelAccum = 0;
					return;
				}
				...
				if (smoothY > snapPts[idx] + 15) {
					// Vẫn còn nội dung phía trên → cuộn tự do
					wheelAccum = 0;
					return;
				}
				if (idx > 0) {
			...
			// Gọi scrollTo với vị trí DOM tuyệt đối — EvergreenSmoothScroll
			// sẽ tự animate mượt đến đó, không cần GSAP animation thêm.
			window.scrollTo(0, snapPts[targetIdx]);
	...
	function initAll() {
		...
		requestAnimationFrame(function () {
			requestAnimationFrame(function () {
				...
				ScrollTrigger.refresh();
				// initSnapScrolling() đã được chuyển sang window.load
				// để đảm bảo chạy sau khi fonts + images đã load xong
			});
		});
	}
	...
	window.addEventListener('load', async function () {
		...
		// Khởi động snap SAU KHI tất cả đã ổn định:
		// fonts đã tải, images đã có kích thước, ScrollTrigger đã refresh xong.
		// Lúc này getDocTop() mới cho vị trí chính xác.
		requestAnimationFrame(function () {
			initSnapScrolling();
		});
	});
