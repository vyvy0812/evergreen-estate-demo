...
(function () {
	...
	function initSnapScrolling() {
		...
		window.addEventListener('wheel', function(e) {
			var now = Date.now();
			var timeSinceLastScroll = now - lastWheelTime;
			lastWheelTime = now;
			...
			var currentIdx = -1;
			var midPoint = scrollY + (winH / 2);

			sections.forEach(function(sec, i) {
				var measureEl = sec.closest('.pin-spacer') || sec;
				var top = measureEl.getBoundingClientRect().top + scrollY;
				var bottom = top + measureEl.offsetHeight;
			...
			var delta = e.deltaY;
			var activeSec = sections[currentIdx];
			var measureEl = activeSec.closest('.pin-spacer') || activeSec;
			...
			if (delta > 0) { 
				if (rect.bottom > winH + 10) {
					return; // Allow native scroll inside the section
				}
				// At bottom boundary: prevent native bleed and accumulate
				e.preventDefault();
				scrollAccumulator += delta;
				if (scrollAccumulator < 20) return; // Threshold to snap
				...
				// At top boundary: prevent native bleed and accumulate
				e.preventDefault();
				scrollAccumulator += delta;
				if (scrollAccumulator > -20) return; // Threshold to snap
			...
			if (targetIdx !== currentIdx) {
				scrollAccumulator = 0; // Reset after trigger
				e.preventDefault(); 
				isAnimating = true;
				lastSnapTime = now; 

				var targetSec = sections[targetIdx];
				var targetMeasureEl = targetSec.closest('.pin-spacer') || targetSec;
				var targetY = targetMeasureEl.getBoundingClientRect().top + scrollY;

				// Use Lenis for TikTok/Reels smooth snap if available
				if (window.EvergreenLenis && window.EvergreenLenis.isActive()) {
					window.EvergreenLenis.scrollTo(targetMeasureEl, { 
						duration: 0.75, 
						lock: true, // Lock scroll during snap
						force: true,
						onComplete: function() {
							isAnimating = false;
						}
					...
					var proxy = { y: window.scrollY || document.documentElement.scrollTop };
					gsap.to(proxy, {
						y: targetY,
						duration: 0.75, 
						ease: "power3.inOut",
	...
	function initAll() {
		...
		requestAnimationFrame(function () {
			requestAnimationFrame(function () {
				...
				// Khởi tạo Snap Scrolling sau khi các thành phần khác đã refresh xong layout
				if (!window.location.search.includes('home-test')) {
					initSnapScrolling();
				}
			});
